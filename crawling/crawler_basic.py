...
def crawling(start_code, finish_code):
    try:
        ...
        with open('../crawling/data/point_code_list.txt', 'r') as text_file:
            point_code_list = text_file.readline().split(",")

		# 항목 : 코드, 제목, 장르, 국가, 상영시간, 감독, 배우, 연령등급, 개봉일, 줄거리, 사진 주소
        sheet.append(["id", "title", "genre", "nation", "playtime", "directors", "actors", "age", "date", "summary", "image"])
        ...
        for code in point_code_list:
            ...
            for a, m in enumerate(movie):
                ...
                sheet.append([code, title.text, genre_str, nation_str, int(playtime.text[0:-2]), directors_str, actors_str, age.text, date_str, summary_str, image])
        ...
        print("에러발생")
        wb.save("../crawling/data/basic2.xlsx")
    finally:
        print("완료")
		
		# 수집된 영화들 코드 리스트 파일 생성
        with open('../crawling/data/basic_code_list.txt', 'w') as f:
