...
def crawling(start_code, finish_code):
    try:
        ...
        for i in range(start_code, finish_code):
            ...
            if is_ok == True:
                j = j + 1
				# point_code_list에 code 저장
                point_code_list.append(code)
			
            print(code, " : ", finish_code - start_code, "개중에", i + 1 - start_code, "번째 영화 체크 중", j, "개의 영화 정보저장 완료")
        # 엑셀 저장
    except:
        print("에러발생")
        wb.save("../crawling/data/point2.xlsx")
    finally:
        print("완료")
		
		# 수집된 영화들 코드 리스트 파일 생성
        with open('../crawling/data/point_code_list.txt', 'w') as f:
            for code in point_code_list:
                if code == point_code_list[-1] :
                    f.write(code)
                else:
                    f.write(code + ",")
